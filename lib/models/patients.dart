...
class Patient {
  ...
  Future<void> addPatientIfNotExists() async {
    //check if patient exists
...
//add new patient data to firestore
Future<void> addPatientData(Patient patient) async {
  //add new patient data to firestore
  await FirebaseFirestore.instance
      .collection('patients')
      .doc(patient.uid)
...
Future<Patient> getPatientByPhone(String phone) async {
  ...
  QuerySnapshot<Map<String, dynamic>> patientData = await FirebaseFirestore
      ...
      .get();
  //return patient
  return Patient.fromMap(patientData.docs.first.data() as Map<String, dynamic>);
}
